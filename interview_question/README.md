...
```
### 反射（reflection）是什么？如何使用反射获取类型信息和修改变量值？
...
### Go语言中struct tag 的作用是什么？
1. 序列化 / 反序列化配置
    1. 指定 JSON/XML 等格式的字段名
    2. 控制字段是否参与序列化
    3. 格式化日期时间等特殊类型
2. 数据校验
    1. 定义字段的校验规则（长度、格式等）
...
### 什么是内存泄漏？Golang 中常见的内存泄漏场景及排查方式是什么？
...
## 五、工程化实践
### 如何使用context包进行 Goroutine 间的上下文管理？
